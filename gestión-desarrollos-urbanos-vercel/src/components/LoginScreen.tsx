...
export default function LoginScreen({ onLoginSuccess, isDarkMode, setIsDarkMode, onOpenHelp }: LoginProps) {
  ...
  // Cargar propietarios en segundo plano para validar accesos por DNI con fallback robusto
  useEffect(() => {
    fetch("/api/clientes")
      .then((res) => {
        if (res.ok) return res.json();
        throw new Error("No se pudieron cargar propietarios desde el servidor");
      })
      .then((data) => {
      ...
      .catch((err) => {
        console.warn("Fallo el fetch de login, usando localStorage o db.json:", err);
        // Fallback a localStorage
        const localDb = localStorage.getItem("fincas_local_db");
        if (localDb) {
          try {
            const parsed = JSON.parse(localDb);
            if (parsed && Array.isArray(parsed.clientes)) {
              setPropietarios(parsed.clientes);
              return;
            }
          } catch (e) {
            console.error("Error al parsear localDb:", e);
          }
        }
        // Fallback a db.json directamente
        if (initialDb && Array.isArray(initialDb.clientes)) {
          setPropietarios(initialDb.clientes);
        }
      });
  }, []);
