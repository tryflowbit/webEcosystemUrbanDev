...
import { calcularMoraDeCuota } from "./lib/calculoMora.js";
...
export default function App() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      console.error("No se pudieron cargar los datos de la base de datos:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFetchDolar = async () => {
    ...
    try {
      ...
      console.error("Surgió un error al obtener cambio:", err);
    } finally {
  ...
  const handleUpdateDolar = async (nuevoValor: number) => {
    try {
      ...
      console.error("No se pudo modificar la cotización manual:", err);
    }
  };
        ...
        <div className="flex h-screen overflow-hidden w-full font-sans">
          ...
          {currentUser?.role === "admin" && (
            <>
              ...
              <aside className="w-16 h-full bg-[#FFFFFF] dark:bg-[#121316] border-r border-[#E5E7EB] dark:border-[#202125] flex flex-col justify-between items-center py-4 z-40 shrink-0 shadow-xs">
              ...
              <AnimatePresence initial={false}>
                {isSidebarExpanded && (
                  <motion.aside
                    ...
                    className="h-full bg-white dark:bg-[#0F1014] border-r border-[#E5E7EB] dark:border-[#202125] flex flex-col justify-between overflow-hidden shrink-0 z-30 shadow-xs"
                  >
              ...
              {!isSidebarExpanded && (
                <button
                  ...
                  className="absolute left-16 top-1/2 -translate-y-1/2 w-8 h-12 bg-white dark:bg-[#121316] hover:bg-stone-50 dark:hover:bg-stone-900 border-t border-r border-b border-l-0 border-[#E5E7EB] dark:border-[#202125] rounded-r-xl flex items-center justify-center z-50 transition-all shadow-xs cursor-pointer text-[#C5A880]"
          ...
          <div className="flex-1 h-full overflow-y-auto flex flex-col bg-[#F0F2F5] dark:bg-[#090A0D] z-10">
            ...
            <header className="h-14 bg-white dark:bg-[#0F1014] border-b border-[#E5E7EB] dark:border-[#202125] px-6 sm:px-8 flex justify-between items-center shrink-0 z-30">
              <div className="flex items-center gap-3">
                <span className="text-[13px] font-bold text-stone-900 dark:text-white flex items-center gap-1">
                  {selectedClient ? (
                    <>
                      <span>Ficha del Propietario</span>
                      <span className="text-stone-400">/</span>
                      <span className="text-sky-650 dark:text-sky-400 font-normal">{selectedClient.apellido}, {selectedClient.nombre}</span>
                    </>
                  ) : activeTab === "resumen" ? (
                    "Resumen del administrador de urbanizaciones"
                  ) : activeTab === "propietarios" ? (
                    "Conjuntos de propietarios"
                  ) : activeTab === "mora" ? (
                    "Informe detallado de mora crítica"
                  ) : (
                    "Configuración cambiaria de referencia (Dólar)"
                  )}
                </span>
              </div>
            ...
            <main className={`flex-1 ${currentUser?.role === "owner" ? "p-6 sm:p-8" : (!isSidebarExpanded ? "pl-14 sm:pl-16 pr-6 sm:pr-8 py-6 font-sans" : "p-6 sm:p-8")} overflow-y-auto w-full max-w-7xl mx-auto space-y-8`}>
                    ...
                    <div className="space-y-6 bg-white dark:bg-[#0F1014] border border-[#E5E7EB] dark:border-[#202125] rounded-xl p-5 sm:p-6 shadow-sm">
                      ...
                      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-6 border-b border-stone-100 dark:border-stone-900">
                        ...
                        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                          ...
                          <div className="flex border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden p-1 bg-stone-50 dark:bg-stone-950 text-[10px] gap-1 font-sans uppercase font-extrabold text-stone-500 select-none">
                            <button
                      ...
                      <div className="relative group">
                        ...
                        <input
                          type="text"
                          placeholder="Filtre por Apellido, Nombre, DNI del Propietario, o Coordenadas de Parcela (Ej. Mza B)..."
                          value={searchTerm}
